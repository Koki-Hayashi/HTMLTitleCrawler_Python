...
また、万が一文字化けするケースを避けるため、
ソースコード中のコメントは英語で記載しています。格好つけたわけじゃないです。どちらかと言うとビビった故の行動です。
...
・拡張性
-> 入力/出力のファイル形式が変わった場合でも対応できるよう、別クラスとして切り出しています。
時間がなかったので実装していませんが、setting.txtなどを用意して、
output_style=tsv|csv
のような形でプロパティを指定することで、コード内で適切なwriterクラスを使用するような拡張は簡単に行えると思います。
その他、一応ファイルがurl以外で指定される場合、titleタグ以外のタグに含まれる文字を調べたい場合などでも、簡単に対応できるよう書いたつもりです。
